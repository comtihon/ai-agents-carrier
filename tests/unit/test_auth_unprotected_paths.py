...
@pytest.mark.parametrize(
    ("method", "path"),
    [
        # Run control lives under the /workflows prefix, so it was never caught
        # by the /api/v1/runs/ exemption. Asserted here to keep it that way.
        ("GET", "/api/v1/workflows/runs/abc-123"),
        ("GET", "/api/v1/workflows/runs/abc-123/trace"),
        ("POST", "/api/v1/workflows/runs/abc-123/approve"),
        ("POST", "/api/v1/workflows/runs/abc-123/terminate"),
        ("DELETE", "/api/v1/workflows/runs/abc-123"),
